...
def create_app(test_config=None):
    ...
    @app.post('/items', response_model=OHMServiceItemsResponse)
    def saveItem(data: str = Body(...)):
        #try:
        ...
        #    print(ex)
        #    return jsonify({'result': 'error', 'error': traceback.format_exception_only(type(ex), ex), 'stack':traceback.format_stack()})
    ...
    class OHMServiceBotsResponse(BaseModel):
        result:str
           
    @app.get('/bots')
    @app.get('/bots/<bot>', response_model=OHMServiceBotsResponse)
    def runBot(bot: str = 'movement'):
        r.rpush('bot', bot)
        return {'result': 'OK'}
